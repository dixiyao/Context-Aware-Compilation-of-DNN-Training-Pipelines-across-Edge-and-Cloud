...
class Client():
    ...
    def send_tensor(self,head,epoch,iters,target,feature, use_Q):
        sendtime=time.time()
        target=target.numpy()
        if not use_Q:
...
class Server():
    ...
    def recieve_tensor(self):
        ...
        target=pickle.loads(self.rec_data[target_pos+6:tensor_pos])
        ...
        return head,epoch,iters,target,feature,use_Q,server_rec,client_send,send_size
